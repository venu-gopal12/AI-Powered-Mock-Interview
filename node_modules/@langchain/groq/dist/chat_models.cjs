...
var ChatGroq = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	...
	constructor(fields) {
		super(fields);
		const apiKey = fields.apiKey || (0, _langchain_core_utils_env.getEnvironmentVariable)("GROQ_API_KEY");
	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "jsonSchema") {
			...
			const jsonSchema = (0, _langchain_core_utils_types.isInteropZodSchema)(schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(schema) : schema;
			...
			if ((0, _langchain_core_utils_types.isInteropZodSchema)(schema)) {
				...
				outputSchema = (0, _langchain_core_utils_json_schema.toJsonSchema)(schema);
			} else outputParser = new _langchain_core_output_parsers.JsonOutputParser();
			llm = this.withConfig({
		...
		return _langchain_core_runnables.RunnableSequence.from([{ raw: llm }, parsedWithFallback]).withConfig({ runName: "ChatGroqStructuredOutput" });
	}
};
