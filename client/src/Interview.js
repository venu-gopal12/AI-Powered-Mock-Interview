...
const Interview = ({ onInterviewEnd }) => {
  ...
  const handleUserMessage = async (text) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/interview', { message: text });
  ...
  const handlePanic = async () => {
    ...
    setIsTyping(true);
    try {
      const res = await axios.post('http://localhost:5000/hint');
  ...
  const endInterview = async () => {
    ...
    if (window.confirm("End the interview and get your score?")) {
      try {
        const res = await axios.post('http://localhost:5000/end-interview', { frontendMessages: messages });
